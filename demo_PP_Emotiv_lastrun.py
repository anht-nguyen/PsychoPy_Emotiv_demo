...
expName = 'demo_PP_Emotiv'  # from the Builder filename that created this script
expInfo = {
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\OneDrive - PennO365\\Documents\\GitHub\\PsychoPy_Emotiv_demo\\demo_PP_Emotiv_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    import random
    random.seed(1)
    sound_1 = sound.Sound('A', secs=0.05, stereo=True, hamming=True,
        name='sound_1')
    ...
    trialComponents = [cortex_rec, text, key_resp]
    ...
    for thisTrial in trials:
        ...
        sound_1.setSound(tone, secs=0.05, hamming=True)
        sound_1.setVolume(1.0, log=False)
        sound_1.seek(0)
        # This is generated by the writeRoutineStartCode
        # keep track of which components have finished
        StimulusComponents = [sound_1, eeg_marker]
        ...
        while continueRoutine:
            ...
            if sound_1.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > sound_1.tStartRefresh + 0.05-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            # if eeg_marker is starting this frame...
            if eeg_marker.status == NOT_STARTED and t >= delay-frameTolerance:
                # keep track of start time/frame for later
                ...
                cortex_obj.inject_marker(value=str(value), label=tone, delta_time=delta_time)
                eeg_marker.start_sent = True
            
            # if eeg_marker is stopping this frame...
